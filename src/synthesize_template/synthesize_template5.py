...
output_folder_1 = './Template5(partialques[...]partialques)/synthesize_input'
output_folder_2 = './Template5(partialques[...]partialques)/synthesize_output'
...
def extract_partial(text):
    '''This function splits the text into a partial beginning and partial ending, ensuring formulas are not split.'''
    
    # Regular expression to match math formulas between \( and \)
    formula_pattern = r'\\\(.*?\\\)'
    ...
    for i, formula in enumerate(formulas):
        ...
        text_with_placeholders = text_with_placeholders.replace(formula, placeholder, 1)
        placeholders.append(placeholder)

    words = text_with_placeholders.split()
    
    num_beginning_words = random.randint(10, 15)  
    num_ending_words = random.randint(5, 15)  
    ...
    for placeholder in placeholders_sorted:
        formula_index = int(placeholder.split("_")[1])
        partial_beginning = partial_beginning.replace(placeholder, formulas[formula_index])
        partial_ending = partial_ending.replace(placeholder, formulas[formula_index])
    ...
    while created_files < total_files:
        ...
        with open(output_file_1, 'w', encoding='utf-8') as f:
            ...
            for i in selected_questions_indices:
                if j==0:
                    _, partial_ending = extract_partial(questions[i])
                    ending_cache = partial_ending 

                    f.write(f"{partial_ending}\n")
                    f.write("\n")
                elif j == len(selected_questions_indices) - 1:
                    partial_beginning, _ = extract_partial(questions[i])
        ...
        with open(output_file_2, 'w', encoding='utf-8') as f:
            ...
            for i in selected_questions_indices:
                if j==0:
                    f.write(f"<question> {ending_cache} <question\>\n")
                    f.write("\n")
                elif j == len(selected_questions_indices) - 1:
                    beginning_cache = modify_string(beginning_cache)
                    f.write(f"{beginning_cache} <question\>\n")
                    f.write("\n")
                else:
