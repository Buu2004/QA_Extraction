...
output_folder_1 = './Template4(ques-ans)/synthesize_input'
output_folder_2 = './Template4(ques-ans)/synthesize_output'
...
def create_combined_mmd_files(questions, answers, solutions, output_folder_1, output_folder_2):
    ...
    while created_files < total_files:
        ...
        num_questions_in_file = random.choice([2, 3, 4, 5, 6, 7, 8, 9, 10])
        ...
        with open(output_file_2, 'w', encoding='utf-8') as f:
            for i in selected_questions_indices:
                ...
                f.write(f"<choices> {ans_list[j]} <choices\><question\>\n")
                f.write("\n")
                
                #f.write(f"<solution> {sol_list[j]} <solution\>\n")
                #f.write("\n")
